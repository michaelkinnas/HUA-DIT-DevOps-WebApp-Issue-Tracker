...
def createTestUsers(sender, **kwargs):
    ...
    # if not User.objects.get(username=env('DJANGO_DEV_NAME')):
    if not User.objects.filter(username=env('DJANGO_DEV_NAME')).exists():
    ...
    if not User.objects.filter(username=env('DJANGO_LEAD_NAME')).exists():
